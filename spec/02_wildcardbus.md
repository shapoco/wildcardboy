...
|3|-|GND|グランド|
|4|GPIO40 (ADC0)|LCA0|アナログ入力 (ホストシェルの ADC に接続)|
...
|17|GPIO25|LCIO12|ロジックカード側の GPIO12|
|18|GPIO26|LCIO13|ロジックカード側の GPIO13|
|19|GPIO28|LCAUX_SDA|I2C データライン|
|20|GPIO29|LCAUX_SCL|I2C クロックライン|
|21|GPIO30|LCUSB_DP|USB D+|
|22|GPIO31|LCUSB_DM|USB D-|
|23|-|LC3V3|3.3V 電源 (常時給電)|
|24|-|LCVSYS|VBUS/VBAT の OR 電源 (スイッチ付き)|

- GPIO 列はホストシェル (RP2350B) 側の GPIO 番号。LCIO12/13 は HSTX 端子確保のため
  ホスト側 GPIO25/26 に割り当てられている (信号名・機能は従来どおり)。
- LC3V3 はホストシェル動作中は常時給電される (イネーブル制御なし)。
- LCVSYS は VBUS と VBAT のダイオード OR 出力 (約 3.0〜5.0V) で、ホストシェルが給電を
  ON/OFF する (ソフトスタート付き)。安定した 5V を必要とするカードはカード側で昇圧すること。
...
ロジックカード側が TF カード I/F を要求する場合は、ホストシェル側の TF カードスロットがロジックカードへ接続される。
...
ロジックカードからホストシェルへのアナログ信号。ホストシェルは GPIO40 (ADC0) で読み取る。

第一の用途はロジックカードからの LCD バックライト輝度制御で、カード側で PWM を RC フィルタで
平滑して LCA0 に出力し、ホストシェルが ADC 値をバックライト PWM (HLCD_BL) に反映する。
使用しないカードは未接続でよい。
