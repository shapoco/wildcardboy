...
LCAUX_SDA/SCL は、ロジックカード上の EEPROM (24LC256, devaddr=0x50) に接続されており、ホストシェル側から読み出すことで、ロジックカードの種類を判別することができる。
...
キーマップに現れないホストシェル側ボタンは、ロジックカードへは送られない。
