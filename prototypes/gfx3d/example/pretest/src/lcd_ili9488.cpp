...
//-----------------------------------------------------------------------------
// Tunables (same as pretest/src/ili9488.cpp)
//-----------------------------------------------------------------------------
...
// and waiting for it to come back proves the SM is idle.
...
void Ili9488::writeBytesAsync(const void* data, uint32_t bytes) {
  if (bytes == 0) return;
  waitDma();
  startDma(data, bytes, false);
}

void Ili9488::swapBytes(uint16_t* buf, uint32_t n) {
  uint32_t* w32 = reinterpret_cast<uint32_t*>(buf);
  uint32_t words = n >> 1;
  for (uint32_t i = 0; i < words; ++i) {
    uint32_t w = w32[i];
    w32[i] = ((w & 0xFF00FF00u) >> 8) | ((w & 0x00FF00FFu) << 8);
  }
  if (n & 1) {
    uint16_t p = buf[n - 1];
    buf[n - 1] = static_cast<uint16_t>((p >> 8) | (p << 8));
  }
}
