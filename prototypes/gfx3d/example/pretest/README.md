...
| `tri` | 三角形数 (カリング後) / トライアングルバッファの容量。`drop` はあふれて破棄した数 |
| `span` | 1 ラインで同時に使用した線分数の最大 / 線分プールの容量。`drop` はあふれて破棄した数 |
| `arena` | 直近のフレームで実際に使用したアリーナのバイト数 (ラインバケット + 三角形 + 線分のピーク) / アリーナのサイズ |

画面の 3 行目は `tri` / `span` / `arena` (使用率 %) で、あふれが起きたときは数値の後ろに `!` が付く。
