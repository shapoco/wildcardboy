...
2. そのラインと交差する三角形 (遠い順) に対して以下の処理を行う
    ...
    2. 線分を線分リストの末尾に追加する。不透明な線分が関与する重なりについては、重なり区間の中央で深度 (スクリーン空間で線形な NDC 深度を補間して求める) を比較し、奥側の線分の重なり部分を削除する。三角形単位のソート順だけに頼らないため、大きなポリゴンと小さなポリゴンの前後関係も正しく解決される。半透明な線分同士は削除されず、リスト順 (遠い順) に合成される
...
透視補正 (スクリーン空間で線形な (u/w, v/w, 1/w) を補間し、ピクセル単位で除算する) を有効にできる (デフォルト無効)。
