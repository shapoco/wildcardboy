...
本体ハードウェアの事前テスト用の仮ファームウェア。TJP カード (ATtiny85 / Tinyjoypad 互換)、
PP1/PP2 カード (Raspberry Pi Pico / Pico 2、PicoPad 互換)、ESPboy カード (ESP8266、ESPboy 互換) を対象とする。
...
ホスト側テスト: `test_host/build.sh` (`cards/TJP` / `PP1` / `ESPboy` の profile.hex のパースとエラー経路)。
